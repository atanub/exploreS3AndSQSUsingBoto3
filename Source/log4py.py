...
class Logger:
    ...
    def get_instance(self, classid="Main", use_cache=TRUE):
        ...
        classid = classid.__class__.__name__  # classid.lower()#lower(classid)
    ...
    def __Logger_parse_options(self, section=SECTION_DEFAULT):
        """ **(private)** Parse main options from _config file. """
    ...
    def __Logger_cache_options(self):
        """ **(private)** Read and cache debug levels for categories from _config file. """
        configfilename = self.__Logger_find_config()
...
class FileAppender:
    def __init__(self, filename, rotation=ROTATE_NONE):
        """ **(private)** Class initalization & customization. """
        self.__FileAppender_filename = filename  # os.path.join(get_homedirectory(), filename).replace('$HOME/','')#sub("\$HOME", get_homedirectory(), filename)
        # print os.path.join(get_homedirectory(), filename).replace('$HOME/','')
